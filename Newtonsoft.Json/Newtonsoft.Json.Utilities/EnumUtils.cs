	...
	internal static class EnumUtils
	{
		...
		private static EnumInfo InitializeValuesAndNames(StructMultiKey<Type, NamingStrategy> key)
		{
			...
			for (int i = 0; i < names.Length; i++)
			{
				...
				string text = (from EnumMemberAttribute a in field.GetCustomAttributes(typeof(EnumMemberAttribute), inherit: true)
				...
				array[i] = ((key.Value2 != null) ? key.Value2.GetPropertyName(text, hasSpecifiedName: false) : text);
			}

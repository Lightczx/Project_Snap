	...
	internal class JsonSerializerInternalReader : JsonSerializerInternalBase
	{
				...
				if (value != null || !contract.IsNullable)
				{
					try
					{
						if (contract.IsConvertable)
						{
							...
							else if (contract.NonNullableUnderlyingType == typeof(DateTime) && (s = (value as string)) != null && DateTimeUtils.TryParseDateTime(s, reader.DateTimeZoneHandling, reader.DateFormatString, reader.Culture, out dt))
							{
		...
		private object CreateObjectUsingCreatorWithParameters(JsonReader reader, JsonObjectContract contract, JsonProperty containerProperty, ObjectConstructor<object> creator, string id)
		{
			...
			foreach (CreatorPropertyContext item in list)
			{
				if (flag && item.Property != null && !item.Presence.HasValue)
				{
					...
					string s;
					PropertyPresence value2 = (value == null) ? PropertyPresence.Null : (((s = (value as string)) == null) ? PropertyPresence.Value : (CoerceEmptyStringToNull(item.Property.PropertyType, item.Property.PropertyContract, s) ? PropertyPresence.Null : PropertyPresence.Value));
		...
		private void EndProcessProperty(object newObject, JsonReader reader, JsonObjectContract contract, int initialDepth, JsonProperty property, PropertyPresence presence, bool setDefaultValue)
		{
			if (presence == PropertyPresence.None || presence == PropertyPresence.Null)
			{
				try
				{
					Required required = (!property.Ignored) ? (property._required ?? contract.ItemRequired ?? Required.Default) : Required.Default;
					switch (presence)
					{

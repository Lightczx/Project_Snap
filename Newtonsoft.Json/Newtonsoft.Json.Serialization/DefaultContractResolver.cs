...
namespace Newtonsoft.Json.Serialization
{
	public class DefaultContractResolver : IContractResolver
	{
		...
		protected virtual List<MemberInfo> GetSerializableMembers(Type objectType)
		{
			...
			IEnumerable<MemberInfo> enumerable = ReflectionUtils.GetFieldsAndProperties(objectType, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic).Where(FilterMembers);
			...
			{
				{
					...
					{
						FieldInfo fieldInfo;
						if ((object)(fieldInfo = (item as FieldInfo)) != null && !fieldInfo.IsStatic)
						{
							list.Add(item);
						}
					}
		...
		{
			PropertyInfo propertyInfo;
			if ((object)(propertyInfo = (memberInfo as PropertyInfo)) != null && propertyInfo.PropertyType.IsGenericType() && propertyInfo.PropertyType.GetGenericTypeDefinition().FullName == "System.Data.Objects.DataClasses.EntityReference`1")
			{
				return false;
			}
			return true;
